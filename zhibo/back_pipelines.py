...
from models import Zhibo_dota2, Zhibo_lol, Zhibo_ls, Zhibo_other, Zhibo_baby,Zhibo_war3, db_connect, create_douyu_table
...
class ZhiboPipeline(object):
	...
	if item['cate'] == "dota2":
		...
		ins = session.query(Zhibo_war3).filter_by(zhubo=item['zhubo']).first()
		if ins: 
			_update_(ins,item)
		else:
        		zhibo = Zhibo_war3(**item)
        		session.add(zhibo)
	else:         	
